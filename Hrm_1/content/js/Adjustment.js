var counter = $("#tableAdjustmentDetail tr").length;
var InEditMode = false;
$(document).ready(function () {

    if ($("#tableAdjustmentDetail tr").length == 0) {
        $("#tableAdjustmentDetail").hide();
    } else {
        $("#tableAdjustmentDetail").show();
        calculateTotal();
    }
    ...
    $('#pform').submit(function (e) {
        ...
        for (var ind = 0; ind < tr.length; ind++) {
            ...
            AdjustmentDetail.uanno = td.eq(0).text();
            AdjustmentDetail.barcode = td.eq(1).text();
            AdjustmentDetail.qty = td.eq(6).text();
            //AdjustmentDetail.FreeQty = td.eq(6).text();
            AdjustmentDetail.retail = td.eq(4).text();
            AdjustmentDetail.cost = td.eq(3).text();
            AdjustmentDetail.discount = 0;
            AdjustmentDetail.amount = td.eq(7).text();
            vm.DocumentDetailList.push(AdjustmentDetail);
        }
...
function Authorize() {
    ...
    for (var ind = 0; ind < tr.length; ind++) {
        ...
        AdjustmentDetail.qty = td.eq(6).text();
        //AdjustmentDetail.FreeQty = td.eq(6).text();
        AdjustmentDetail.retail = td.eq(7).text();
        AdjustmentDetail.cost = td.eq(8).text();
        AdjustmentDetail.discount = td.eq(9).text();
        AdjustmentDetail.amount = td.eq(10).text();
        AdjustmentDetail.Totalamount = $("#txt_totalAmount").val();
        AdjustmentDetail.TotalQty = $("#txt_TotalQty").val();
        vm.DocumentDetailList.push(AdjustmentDetail);
    }
    if (vm.DocumentDetailList.length == 0) {
        $('#btn_authorize').removeAttr("disabled");
...
function showTable(e) {
    ...
    var CurrentQty = selectedTr.eq(7).text();
    ...
    if (location == "") {
        ...
        if (index != -1) {
            ...
            var Quantity = parseFloat(td.eq(6).text());
...
function ClearForm(flag) {
    ...
    if (docno == "") {
        $.ajax({
            url: "/StockAdjustment/GenerateBarcode",
            method: "get",
        })
          .done(function (obj) {
...
function calculateTotal() {
    ...
    for (var ind = 0; ind < tr.length; ind++) {
        ...
        var rowQty = parseFloat(td.eq(6).text());
        if (rowQty <= 0) {
            toastr.error("Qty should be grater than 0")
            return;
        }
        var rowCost = parseFloat(td.eq(3).text());

...
function Authorize() {
    ...
    $.ajax({
        url: "/api/AuthorizePurchaseReturn",
        method: "post",
        data: vm
    })
        .done(function (obj) {
        ...
        if (index != -1) {
            ...
            $("#tblbody").append(
                "<tr id=trrr" + counter + ">" +
                    ...
                    "<td class='changeable' contenteditable='true'>" + freeQty + "</td>" +
                    "<td class='changeable' contenteditable='true' style='display:none;'>" + retail + "</td>" +
...
//    for (var ind = 0; ind < tr.length; ind++) {
//        var td = $(tr[ind]).find('td');
//        totalAmount = parseFloat(totalAmount) + parseFloat(td.eq(10).text());
//        TotalQty = parseFloat(TotalQty) + parseFloat(td.eq(5).text());
//        TotalDiscount = parseFloat(TotalDiscount) + parseFloat(td.eq(9).text());
//    }
...
function ClearForm(flag) {
  ...
  //var locationid=  $("#txt_location").val();
  //$("#ddl_location").val(locationid);
    $("#txt_docNo").val("");
    $("#tablePurchaseDetail").hide();
    $("#txt_totalAmount").val("");
    $("#txt_qty").val("0");
    ...
    if (docno == "") {
        $.ajax({
            url: "/PurchaseReturn/GetCode",
            method: "get",
            //  data:doctype
        }).done(function (obj) {
            $("#new_DocNo").val(obj);
              $("#txt_docNo").val(docno);
              $("#txt_docDisplayNo").text(obj);
